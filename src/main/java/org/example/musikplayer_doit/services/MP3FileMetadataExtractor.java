...
public class MP3FileMetadataExtractor {
    ...
    public void extractTagFromMp3(Song[] songArray) throws NullPointerException {
        ...
        for (var s : songArray) {
            try {
                File file = new File(s.getPath().toUri());
...
//            metadataMap.put("sampleRate", audioFile.getAudioHeader().getSampleRate());
    ...
    public void extractTagFromMp3(Song song) throws NullPointerException {

        try {
            File file = new File(song.getPath().toUri());
            AudioFile audioFile = AudioFileIO.read(file);
            Tag tag = audioFile.getTag();
            if (tag != null) {
                song.addMetadata("Album", tag.getFirst(FieldKey.ALBUM));
                //song.addMetadata("Title", tag.getFirst(FieldKey.TITLE));
                song.addMetadata("Artist", tag.getFirst(FieldKey.ARTIST));
                song.addMetadata("Genre", tag.getFirst(FieldKey.GENRE));
                song.addMetadata("Length", String.valueOf(audioFile.getAudioHeader().getTrackLength()));


                System.out.println("Scanned metadata for file" + song.getPath() + "\nMetadata scanned: " +
                        tag.getFirst(FieldKey.ALBUM)
                        //+tag.getFirst(FieldKey.TITLE)
                        + tag.getFirst(FieldKey.ARTIST)
                        + tag.getFirst(FieldKey.GENRE));
